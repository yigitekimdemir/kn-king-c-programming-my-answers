...
int main(void)
{
    int n, nextNumber = 1; //assign the first number as 1
    ...
    int magicSquare[n][n]; //array declaration based on user input

    for (int i = 0; i < n; i++) //set all array values to false
    {
        for (int j = 0; j < n; j++)
            magicSquare[i][j] = false;
    }

    int j = n/2; //initial position of '1'
    int i = 0;

    magicSquare[i][j] = nextNumber; //setting the number '1' into its place to begin the software
    ...
    for (;;) //loops until break kicks in
    {
        ...
        if (magicSquare[i][j] == false) //if the place number needs to go is free
        {
            magicSquare[i][j] = nextNumber; //put it there
        }
        else if (magicSquare[i][j] != false) //if the place number needs to go is not free
...

        {
            ...
            magicSquare[i][j] = nextNumber; //at last!
        }
